...
import { cn, formatCurrencyShort } from "@/lib/utils/format";
...
const navLinks = [
  ...
  { href: "/categoria/materiales-de-oficina", label: "Oficina" },
      ...
      <div className="hidden bg-brand-900 text-white lg:block">
        <div className="container-page flex h-9 items-center justify-between text-[12.5px]">
          ...
          <div className="flex items-center gap-5">
            <a href={telHref} className="inline-flex items-center gap-1.5 text-brand-100 transition-colors hover:text-white">
      ...
      <header className="sticky top-0 z-50 border-b border-line bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/85">
        ...
        <div className="container-page flex h-16 items-center gap-3 lg:h-[72px] lg:gap-6">
          ...
          <div className="ml-auto flex items-center gap-0.5 lg:ml-0 lg:gap-1">
            ...
            <Link
              href="/cotizacion"
              aria-label={`Cotización empresarial${quoteReady && quoteCount ? `, ${quoteCount} artículos` : ""}`}
              className="relative hidden h-10 items-center gap-2 rounded-lg px-3 text-sm font-medium text-ink transition-colors hover:bg-slate-100 sm:inline-flex"
            >
            ...
            <button
              ...
              aria-label={`Abrir carrito${cartReady && cartCount ? `, ${cartCount} artículos` : ""}`}
            >
              <span className="relative">
                <Icon name="cart" size={20} />
                <CountBadge count={cartReady ? cartCount : 0} />
              </span>
